...
import { forgotPassword } from "../pages/api/forgotpass"; // Asumiendo que tienes la lógica en forgotpass.js
...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const data = await forgotPassword(email); // Llamada al servicio para enviar el código

      if (data.success) {
        setAlerta({ tipo: "success", mensaje: data.message });

        // Redirige a la página de restablecer contraseña, pasando el correo como parámetro
